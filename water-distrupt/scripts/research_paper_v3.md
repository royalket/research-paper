...
<!-- P3 | Role: Design — three dimensions and scoring logic -->
The IDI comprises three dimensions, each scored 0–3 where higher scores indicate greater lock-in. The original design had four dimensions, but Dimensions 1 (source lock-in) and 3 (system dependency) correlated at r = 0.873 in NFHS-5 — both scored high for piped households and zero for tube-well households, because both were asking "how dependent are you on a system you do not control?" from slightly different angles. Keeping them separate double-counted the same signal. We merge them into Dimension A (Source Risk).

Dimension A (Source Risk) combines backup availability and system dependency into one score: piped primary with no non-piped backup scores 3, representing maximum centralised dependency with no escape route; tanker or bottled water primary scores 2, representing pure market dependency; community RO plant or protected well or spring scores 1, representing semi-managed local access; tube well, own well, rainwater, or surface water scores 0, representing self-sufficient access that no external system controls. Dimension B (Access Complexity) measures fetching experience: households with water on-premises — derived from NFHS-5 variable hv204 = 996, indicating the source is at or within the dwelling — score 3, reflecting zero fetching experience; off-premises households score 2, 1, or 0 depending on whether the collection point is in the yard, within 15 minutes, or 15 minutes or more away. Dimension C (Piped Coping Deficit) applies only to piped households and measures their buffer capacity: a buffer score is computed as wealth quintile contribution (Q1–Q2 → 0, Q3 → 1, Q4 → 2, Q5 → 3) plus refrigerator ownership plus vehicle ownership, clipped to [0, 3]; the coping deficit score is 3 minus the buffer, so poorest piped households with no assets score 3 and richest piped households with full assets score 0. Non-piped households score 0 on Dimension C regardless of assets, because they already fetch regularly and maintain a practiced routine — the coping deficit concept applies only to households dependent on a tap that can fail.
